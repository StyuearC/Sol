...
function DFunctions.CreateButton(ButtonName, Name, Size1, Size2, ScriptLogic, CircleMode, Family)
    ...
    -- Live-sync transparency from getgenv().ButtonTransparency
    task.spawn(function()
        while frame.Parent do
            local bt = getgenv().ButtonTransparency or {}
            pcall(function()
                frame.BackgroundTransparency = math.clamp(bt.Background or 0.7, 0, 1)
                stroke.Transparency          = math.clamp(bt.Stroke     or 0.0, 0, 1)
                button.TextTransparency      = math.clamp(bt.Text       or 0.0, 0, 1)
                toggle.BackgroundTransparency = math.clamp(bt.Toggle    or 0.3, 0, 1)
                toggle.TextTransparency      = math.clamp(bt.Text       or 0.0, 0, 1)
            end)
            task.wait(0.1)
        end
    end)
...
-- Emote Slot Cycle floating button (follows exact LagSwitch pattern)
local _emoteSlot = 1

Tabs.Movement:AddSpace()

local Toggle = Tabs.Movement:AddToggle("EmoteSlotCycleBtn", { Title = "Emote Slot Cycle (Button)", Default = false })

Toggle:OnChanged(function(State)
    if State then
        DFunctions.CreateButton("EmoteSlotBtn", "#" .. tostring(_emoteSlot), 0.15 + DConfiguration.Settings.GuiScale.EmoteSlot, 0.1 + DConfiguration.Settings.GuiScale.EmoteSlot, function(btn)
            _emoteSlot = (_emoteSlot % 6) + 1
            pcall(function() btn.Text = "#" .. tostring(_emoteSlot) end)
            pcall(function()
...
Tabs.Movement:AddKeybind("EmoteSlotKeybind", {
    Title    = "Emote Slot Cycle Keybind",
    Mode     = "Toggle",
    Default  = "E",
    Callback = function(Value)
        if Value then
...
-- Player Effects (Workspace.Game.Players)

do
    Tabs.Visual:AddSection("Player Effects")

    Tabs.Visual:AddSpace()

    local _effectStore = {}

    local function getGamePlayerList()
        local list = { "All" }
        pcall(function()
            for _, child in ipairs(workspace.Game.Players:GetChildren()) do
                if Players:FindFirstChild(child.Name) and Players:FindFirstChild(child.Name) ~= LocalPlayer then
    ...
    local GPDrop = Tabs.Visual:AddDropdown("PlayerEffectSelect", {
        Title   = "Select Player(s)",
        Values  = getGamePlayerList(),
        Multi   = true,
        Default = {},
    })
    local _gpSelected = {}
    GPDrop:OnChanged(function(Value) _gpSelected = Value end)

    Tabs.Visual:AddSpace()

    local _effectType = "Rainbow"
    local EffectTypeDrop = Tabs.Visual:AddDropdown("PlayerEffectType", {
        Title   = "Unusual Effect Type",
        Values  = { "Rainbow", "Fire", "Sparkle", "Neon Glow", "Smoke", "Force Field" },
        Multi   = false,
        Default = 1,
    })
    EffectTypeDrop:OnChanged(function(Value) _effectType = Value end)

    local function applyEffect(char, effectType, pName)
        if not char then return end
        local hrp = char:FindFirstChild("HumanoidRootPart") or char:FindFirstChild("Head")
        if not hrp then return end
        if not _effectStore[pName] then _effectStore[pName] = {} end
        ...
        if effectType == "Fire" then
            if not hrp:FindFirstChildOfClass("Fire") then
                ...
                f.Parent = hrp
                table.insert(_effectStore[pName], f)
            end

        elseif effectType == "Sparkle" then
            if not hrp:FindFirstChildOfClass("Sparkles") then
                local sp = Instance.new("Sparkles")
                sp.SparkleColor = Color3.fromRGB(96, 205, 255)
                sp.Parent = hrp
                table.insert(_effectStore[pName], sp)
            end

        elseif effectType == "Smoke" then
            if not hrp:FindFirstChildOfClass("Smoke") then
                local sm = Instance.new("Smoke")
                sm.Color = Color3.fromRGB(200, 200, 200)
                sm.Opacity = 0.5; sm.RiseVelocity = 4
                sm.Parent = hrp
                table.insert(_effectStore[pName], sm)
            end

        elseif effectType == "Force Field" then
            if not hrp:FindFirstChildOfClass("ForceField") then
                local ff = Instance.new("ForceField")
                ff.Visible = true
                ff.Parent = char
                table.insert(_effectStore[pName], ff)
            end

        elseif effectType == "Neon Glow" then
            local saved = {}
            for _, part in ipairs(char:GetDescendants()) do
                if part:IsA("BasePart") then
                    pcall(function()
                        saved[part] = { mat = part.Material, col = part.Color }
                        part.Material = Enum.Material.Neon
                        part.Color    = Color3.fromRGB(0, 200, 255)
                    end)
                end
            end
            table.insert(_effectStore[pName], { type = "NeonRestore", saved = saved })

        elseif effectType == "Rainbow" then
            local active = true
            local conn
            conn = RunService.Heartbeat:Connect(function()
                if not active or not char or not char.Parent then
                    conn:Disconnect()
                    return
                end
            ...
            table.insert(_effectStore[pName], { type = "RainbowConn", conn = conn, active = function() active = false end })
        end
    end

    Tabs.Visual:AddButton({
        Title       = "Apply Effect",
        Description = "Apply unusual effect to selected player(s)",
        Callback    = function()
            local applyAll = _gpSelected["All"] == true
            local count = 0
            for _, p in ipairs(Players:GetPlayers()) do
                if p ~= LocalPlayer and p.Character then
                    if applyAll or _gpSelected[p.Name] == true then
                        applyEffect(p.Character, _effectType, p.Name)
                        count += 1
                    end
                end
            end
            if count > 0 then
                Fluent:Notify({ Title = "Player Effects", Content = _effectType .. " applied to " .. count .. " player(s)", Duration = 3 })
            else
                Fluent:Notify({ Title = "Player Effects", Content = "No players selected or found", Duration = 3 })
            end
        end,
    })
    ...
    Tabs.Visual:AddButton({
        ...
        Description = "Remove effects from selected player(s)",
        Callback    = function()
            local applyAll = _gpSelected["All"] == true
            local count = 0
            for name, effects in pairs(_effectStore) do
                local target = Players:FindFirstChild(name)
                if applyAll or (target and _gpSelected[name] == true) then
                    for _, e in ipairs(effects) do
                        pcall(function()
                            if typeof(e) == "Instance" then
                                e:Destroy()
                            elseif type(e) == "table" then
                                if e.type == "NeonRestore" then
                                    for part, data in pairs(e.saved) do
                                        pcall(function()
                                            part.Material = data.mat
                                            part.Color    = data.col
                                        end)
                                    end
                                elseif e.type == "RainbowConn" then
                                    e.active()
                                    e.conn:Disconnect()
                                end
                            end
                    ...
                    _effectStore[name] = nil
                    count += 1
                end
            end
            Fluent:Notify({ Title = "Restore Effects", Content = count .. " player(s) restored", Duration = 3 })
        end,
    })

    Tabs.Visual:AddButton({
        Title       = "Restore All Effects",
        Description = "Remove all unusual effects from every player",
        Callback    = function()
            local count = 0
            for name, effects in pairs(_effectStore) do
                for _, e in ipairs(effects) do
            ...
            _effectStore = {}
            Fluent:Notify({ Title = "Restore All", Content = count .. " player(s) restored", Duration = 3 })
        end,
    })
end
...
do
    ...
    Tabs.Visual:AddInput("NextbotDecalId", {
        ...
        Placeholder = "Enter rbxassetid number",
        Numeric     = false,
        Finished    = false,
        Callback    = function(Value)
...
Tabs.Settings:AddSection("Floating Button Appearance")

Tabs.Settings:AddSlider("BtnTransBackground", {
    Title    = "Button Background Transparency",
    Default  = 70,
    Min      = 0,
    Max      = 100,
    Rounding = 0,
    Callback = function(Value)
        getgenv().ButtonTransparency.Background = Value * 0.01
    end,
})

Tabs.Settings:AddSlider("BtnTransStroke", {
    Title    = "Button Border (UIStroke) Transparency",
    Default  = 0,
    Min      = 0,
    Max      = 100,
    Rounding = 0,
    Callback = function(Value)
        getgenv().ButtonTransparency.Stroke = Value * 0.01
    end,
})

Tabs.Settings:AddSlider("BtnTransText", {
    Title    = "Button Text Transparency",
    Default  = 0,
    Min      = 0,
    Max      = 100,
    Rounding = 0,
    Callback = function(Value)
        getgenv().ButtonTransparency.Text = Value * 0.01
    end,
})

Tabs.Settings:AddSlider("BtnTransToggle", {
    Title    = "Button Icon Transparency",
    Default  = 30,
    Min      = 0,
    Max      = 100,
    Rounding = 0,
    Callback = function(Value)
        getgenv().ButtonTransparency.Toggle = Value * 0.01
    end,
})
