...
Fluent:RegisterCustomTheme("SolasmotaryX", {
...
-- Internal library managers (from FluentLite)
...
-- Cola usage leaderboard table (client-visible only)
...
function DFunctions.BHOPFunction()
    ...
    if DConfiguration.Misc.MovementModification.BHOP.JumpType == "Simulated" then
        if grounded and (now - DConfiguration.Misc.MovementModification.BHOP.lastTick) > debounce then
            ...
            -- Fire JumpEnded to reset Movement module's per-tap debounce
            task.delay(0.08, function()
                if _bhopJumpEndedEvent then
                    pcall(function() _bhopJumpEndedEvent:Fire() end)
                end
            end)
        end
    end
...
-- Module-conflict patch: keeps Jumping state enabled and provides Hold BHOP support.
-- _bhopHoldActive tracks whether Space / mobile JumpButton is currently held.
local _bhopHoldActive     = false
local _bhopLastJumpReq    = 0
local _bhopMobileHooked   = false
local _bhopJumpEndedEvent = nil

-- Find JumpEnded BindableEvent in PlayerScripts (used to reset module debounce)
task.spawn(function()
    local ps = LocalPlayer:FindFirstChild("PlayerScripts")
    if not ps then return end
    task.wait(3)
    for _, obj in ipairs(ps:GetDescendants()) do
        if obj:IsA("BindableEvent") and obj.Name == "JumpEnded" then
            _bhopJumpEndedEvent = obj
            break
        end
    end
end)
...
Tabs.Main:AddButton({
    Title       = "Use Cola",
    Description = "Use a cola item instantly",
    Callback    = function() DFunctions.UseCola() end,
})
...
do
    ...
    -- BoolValues
    for _, name in ipairs({"BoomboxEnabled","CanBeCarried","FPShadows","Jumpscares","LowQuality","Ragdolls","Shadows","SprintViewmodels"}) do
...
Tabs.Movement:AddSection("Cola Speed")

Tabs.Movement:AddSpace()

local Toggle = Tabs.Movement:AddToggle("ColaSpeedEnabled", { Title = "Cola Speed Boost", Default = false })
Toggle:OnChanged(function(State)
    DConfiguration.Main.Cola.SpeedEnabled = State
end)

Tabs.Movement:AddInput("ColaSpeedAmount", {
    ...
    Finished    = false,
    Callback    = function(Value)
        DConfiguration.Main.Cola.SpeedAmount = tonumber(Value) or 10
    end,
})

Tabs.Movement:AddInput("ColaSpeedDuration", {
    Title       = "Boost Duration (seconds)",
    Default     = "5",
    Placeholder = "5",
    Numeric     = false,
    Finished    = false,
    Callback    = function(Value)
        DConfiguration.Main.Cola.SpeedDuration = tonumber(Value) or 5
    end,
})
